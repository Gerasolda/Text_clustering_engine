...
int main() {
    ...
    while(getline(cin, line) && !line.empty()) {
        ...
        while (iss >> word) {
            words.push_back(normalize(word)); // нормализуем слово
        }
    }
    // чтобы применить dsu каждому слову присовим его номер
    for (string w : words) {
    ...
    // делаем dsu на этих словах, также используем скользящее окно размером k
    int n = u_words.size();
    sz.assign(n, 1);
    par.assign(n, 0);
    ...
    for (int idx = 0; idx < (int)words.size(); idx++) {
        ...
        dq.push_back(curr); // скользящее окно размером k
        while ((int)dq.size() > k) {
            dq.pop_front();
        }

        vector<string> a; // не будем учитывать пустые слова
    ...
    unordered_map<string, int> freq; // частоты
    for (string w : words) {
        if (!w.empty()) freq[w]++;
    }

    unordered_map<int, vector<string>> groups; // делаем группы
    for (auto [w, id] : ids) {
        int rot = root(id);
    ...
    for (auto& [root, words] : groups) {
        ...
        string best; // выбираем самое короткое и лексикографическое слово
        for (const string& w : words) {
            if (w.length() > 1) {
    ...
    sort(res.begin(), res.end(), comp); // сортировка с компаратором
